...
int main(int argc,char *argv[]){
	...
	for(int i=0; i< NUMBER_OF_RANDOM; i++){
		...
		if(RandomNumbers[i] < DOUBLE_TRIPLE_CHOICE_NUMBER){
			...
			printf("The function doubleGeneratedRandomNumber() was executed at loop itteration %d to generate  : %d\n", i,RandomNumbers[i]);
		}
			
		else {
			RandomNumbers[i] = tripleGeneratedRandomNumber(RandomNumbers[i]);	
			printf("The function tripleGeneratedRandomNumber() was executed at loop itteration %d to generate : %d\n", i,RandomNumbers[i]);		
		}	
